...
class GenerateHeatmap():
    def __init__(self, output_res, num_parts):
        ...
        self.sigma = sigma
        size = 6*sigma + 3
        x = np.arange(0, size, 1, float)  # numpy.arange(start, stop, step, dtype) -> numpy.ndarray
        y = x[:, np.newaxis]  # make it as column vector by inserting an axis along second dimension
...
class Dataset(torch.utils.data.Dataset):
    ...
    def loadImage(self, idx):
        ...
        for i in range(np.shape(orig_keypoints)[1]):
            if orig_keypoints[0, i, 0] > 0:
                orig_keypoints[0, i, :2] = img.transform(orig_keypoints[0, i, :2], c, s,
    ...
    def loadOrigImage(self, idx):
        ...
        mat = img.get_transform(c, s, (self.input_res, self.input_res), 0)[:2]
        mat_mask = img.get_transform(c, s, (self.output_res, self.output_res), 0)[:2]
        inp = cv2.warpAffine(orig_img, mat, (self.input_res, self.input_res)).astype(np.float32) / 255
        keypoints[:, :, 0:2] = img.kpt_affine(keypoints[:, :, 0:2], mat_mask)

        ## generate heatmaps on outres
        heatmaps = self.generateHeatmap(keypoints)

        return inp.astype(np.float32), heatmaps.astype(np.float32), keypoints
...
def init(config):
    ...
    import data.mpii.mpii_data_handler as ds
    ds.init()
    ...
    loaders = {}
    for key in dataset:
        loaders[key] = torch.utils.data.DataLoader(dataset[key], batch_size=batchsize, shuffle=True, num_workers=config['train']['num_workers'], pin_memory=True)
